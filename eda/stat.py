...
def corrRelation(total_df):
    ...
    st.markdown("### 상관관계 계수 및 검정 \n"
                "- 계수를 확인한다. \n")
    seoul_coef = pg.corr(corr_df['BLDG_AREA'], corr_df['OBJ_AMT'])["r"].values[0]

    st.dataframe(pg.corr(corr_df['BLDG_AREA'], corr_df['OBJ_AMT']).round(3), use_container_width=True)
    st.markdown(f"- 상관계수는 {seoul_coef:.2f} 이며 건물면적이 증가할 때 마다, 물건금액도 같이 증가하는 경향성을 나타나는 것을 확인하라 수 있다. \n"
